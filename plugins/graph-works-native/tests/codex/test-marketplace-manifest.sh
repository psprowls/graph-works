...
# The icons must resolve. Their filenames are child 3's call, so assert the
# manifest's own values point at files that exist rather than hardcoding names.
for key in ("composerIcon", "logo"):
    rel = manifest.get("interface", {}).get(key)
    if not rel:
        raise AssertionError(f"interface.{key} must be set")
    if not (repo_root / rel.lstrip("./")).exists():
