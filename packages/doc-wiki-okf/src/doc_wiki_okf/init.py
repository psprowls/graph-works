"""Install `doc-wiki-okf`'s own files into an OKF v0.2 bundle, additively.

Three acts, all idempotent: `plan_scaffold` -> `plan_install` -> one `log.md`
line when the install actually wrote something. In a bundle other tier-3
...
**There is no `SEED_ONLY` here**, matching `work-tracker-okf`: this package
ships no file another package's human is expected to edit, so all twelve
members are owned templates -- created when absent, skipped when
byte-identical, refused per file when they differ. `index.md`, `log.md` and
`_tags.yaml` belong to `okf_ext.bundle`'s scaffold, not here.
"""
...
def plan_install(root: str | Path, *, declarations_dir: str | Path | None = None) -> InstallPlan:
    """Plan this package's twelve files into *root*.
    ...
    `doc-wiki-okf`'s. No `seed_only=` -- this package owns every member it
    ships.
    """
    ...
    *,
    ...
    if install_result.written:
        ...
        if log_document is not None and log_document.parse_error is None:
            from doc_wiki_okf import __version__  # deferred: no dependency on __init__.py's assignment order

            entry = f"installed by doc-wiki-okf/{__version__}"
            try:
