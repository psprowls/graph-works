# work

Human catalog note preserved outside the generated region.

<!-- graph-works:work-items:start -->
- [Epic: The stalled decomposition](epic-stalled-decomposition.md) — open · execute
- [Spike: A Spike used as a parent](spike-wrong-parent-type.md) — resolved · done
- [Feature: A Feature at a bug-prefixed path](bug-superseded-orphan.md) — superseded · not started
- [TechDebt: Mitigated with no mitigation](tech-debt-mitigated.md) — mitigated · execute
- [TestGap: Wontfix with no rationale](test-gap-wontfix.md) — wontfix · not started
- [Bug: Cycle A](bug-cycle-a.md) — open · not started
- [Bug: Cycle A duplicate](bug-cycle-a.md) — open · not started
- [Bug: Cycle B](bug-cycle-b.md) — open · not started
- [Feature: Owner of a nested release](feature-parent-cycle-a.md) — open · not started
- [Epic: The ledger at finish](epic-ledger-at-finish.md) — in-progress · finish
- [Nested child](spike-wrong-parent-type/children/feature-accepted-no-plan.md) — accepted · design
- [Gone](gone.md) — open · not started
- [Bug: an unbalanced ] in link text](bug-unreadable-entry.md) — open · not started
<!-- graph-works:work-items:end -->
