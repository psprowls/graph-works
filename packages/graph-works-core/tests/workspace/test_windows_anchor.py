...
def test_link_translates_error_invalid_function_into_the_named_refusal(
    ...
    try:
        ...
        def _raise_invalid_function(*args: object, **kwargs: object) -> None:
            # `winerror` is set by assignment, never by OSError's 4th positional
            # argument: CPython only maps that argument to the attribute on
            # Windows builds, so passing it here would leave the attribute
            # absent on a POSIX host and the translation under test unreached.
            error = OSError(0, "Incorrect function")
            error.winerror = anchors.ERROR_INVALID_FUNCTION  # type: ignore[attr-defined]
            raise error
...
def test_link_translates_error_not_supported_into_the_named_refusal(
    ...
    try:
        ...
        def _raise_not_supported(*args: object, **kwargs: object) -> None:
            # See `_raise_invalid_function` above: assignment, not the 4th
            # positional argument, is what makes `winerror` readable on POSIX.
            error = OSError(0, "Not supported")
            error.winerror = anchors.ERROR_NOT_SUPPORTED  # type: ignore[attr-defined]
            raise error
