...
def test_orchestrate_keeps_no_compatibility_re_export_for_resolve_repo():
    """`resolve_repo` MOVED to `workspace/repos.py`; it was not copied. The
    function had one caller, so a re-export here would be a second name for
    the same thing with nothing asking for it."""
    assert "resolve_repo" not in orchestrate.__all__
