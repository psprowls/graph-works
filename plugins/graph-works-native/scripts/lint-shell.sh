#!/usr/bin/env bash
#
# Adopted verbatim from plugins/graph-works/scripts/lint-shell.sh (the
# vendored subtree's own copy) as a one-time adoption for the native plugin
# root, which has no such script of its own.
#
