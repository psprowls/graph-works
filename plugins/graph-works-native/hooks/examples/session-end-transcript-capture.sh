...
# plugins/graph-works-native/hooks/examples -> repo root is four levels up. A
# hook registered by graph-works receives GRAPH_WORKS_PYTHON, binding it to the
# interpreter that installed the packages. The root is the source-checkout
# fallback for a manually copied example hook.
ROOT="${GRAPH_WORKS_ROOT:-$(cd "${SCRIPT_DIR}/../../../.." && pwd)}"
