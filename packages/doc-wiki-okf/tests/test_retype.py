...
def bundle_root(tmp_path: Path) -> Path:
    ...
    build_bundle(
        root,
        {
            "docs/explanations/why-it-works": _page("Explanation", "Why it works", "docs/explanations/why-it-works"),
            "docs/reference/flags": citing,
        },
    )
    return root
...
def test_the_round_trip_moves_the_page_rewrites_the_type_and_repairs_the_link(bundle_root: Path) -> None:
    ...
    moved = bundle_root / "docs" / "reference" / "why-it-works.md"
    assert moved.is_file()
    assert not (bundle_root / "docs" / "explanations" / "why-it-works.md").exists()
    assert load(moved).fm.type == "Reference"

    citing = (bundle_root / "docs" / "reference" / "flags.md").read_text(encoding="utf-8")
...
def test_the_type_write_lands_even_though_the_move_is_planned_first(bundle_root: Path) -> None:
    ...
    assert load(bundle_root / "docs" / "tutorials" / "why-it-works.md").fm.type == "Tutorial"
...
def test_a_rubric_type_with_no_installed_schema_is_refused(bundle_root: Path, tmp_path: Path) -> None:
    ...
    (partial / "Explanation.schema.json").write_text(
        ...
        ' "x-okf-directory": "docs/explanations/"}',
        encoding="utf-8",
    )
...
def test_the_moves_own_refusals_stay_on_the_move_and_still_sink_ok(bundle_root: Path) -> None:
    ...
    (bundle_root / "docs" / "reference" / "why-it-works.md").write_text(
        "---\ntype: Reference\ntitle: Taken\ndescription: Taken.\n---\n\n## Summary\n\nWords.\n",
        encoding="utf-8",
    )
    bundle = load_bundle(bundle_root)
...
def test_a_wikilink_only_vault_and_a_quiet_vault_produce_different_retype_plans(tmp_path):
    ...
    citing = (
        "---\ntype: Reference\ntitle: Flags\ndescription: Flags.\n---\n\n"
        "## Summary\n\nSee [[docs/explanations/why-it-works]] for the reason.\n"
    )
    ...
    for name, citing_text in (("loud", citing), ("quiet", quiet)):
        ...
        build_bundle(
            root,
            {
                "docs/explanations/why-it-works": _page(
                    "Explanation", "Why it works", "docs/explanations/why-it-works"
                ),
    ...
    assert [entry.target for entry in loud.move.stranded] == ["docs/explanations/why-it-works.md"]
