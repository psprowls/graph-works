...
# Local edits (graph-works): this fork repo IS the plugin, root to root.
# obra ships its plugin nested at plugins/superpowers/ inside a multi-plugin
# marketplace repo; we use DEST_REL="." because the spike proved Codex only
# recognises .codex-plugin/plugin.json sitting directly at the marketplace root
# and does not recurse (evidence/codex.md, Attempt 2).
...
# Local edit (graph-works): --delete-excluded REMOVED.
